...
function Splash({ accent, fading }) {
  ...
  useEffect(() => {
    const t = setTimeout(() => setFlare(true), 6100);
    return () => clearTimeout(t);
  }, []);
  // ---- Splash V4.8 scene: ONE canvas, ONE rAF loop, zero per-frame React state ----
  // Everything that moves (starfield + the candlestick stream spiralling into the event horizon) is
  // drawn on a single 2D canvas. The stars are generated once from a fixed seed (mulberry32), so a
  // resize or a re-render never re-shuffles the sky. The black hole itself stays a CSS composite over
  // the untouched photo (see .splash2-bh-* below) \u2014 canvas only handles the parts that need per-frame
  // physics. Full teardown on unmount: rAF cancelled, resize listener removed, context dropped.
  useEffect(() => {
    ...
    // stars: three loose depth bands, each with its own size / brightness / drift rate, plus an
    // independent slow twinkle phase so nothing pulses in sync.
    const STAR_N = 160;
    const stars = [];
    for (let i = 0; i < STAR_N; i++) {
      const depth = 0.12 + rnd() * 0.88;
      stars.push({
        px: rnd(), py: rnd(),
        depth,
        r: 0.35 + depth * 1.15,
        ...
        ph: rnd() * Math.PI * 2,
        orbit: rnd() < 0.72,
        vx: (rnd() - 0.5) * 0.0055 * depth,
        vy: (rnd() - 0.5) * 0.0055 * depth,
        spin: (0.006 + rnd() * 0.020) * depth,
        fade: rnd() < 0.28
      });
    }
    // candles: a seeded random walk, drawn as a stream whose tail lags the head along the spiral.
    const CANDLE_N = 32;
    const candles = [];
    let price = 0;
    for (let i = 0; i < CANDLE_N; i++) {
      const o = price;
      price = o + (rnd() - 0.40) * 1.05;
      candles.push({ o, c: price, hi: Math.max(o, price) + rnd() * 0.5, lo: Math.min(o, price) - rnd() * 0.5 });
    }
    ...
    const resize = () => {
      ...
      cx = w * 0.47;
      cy = h * 0.41;
      R0 = Math.hypot(w * 0.62, h * 0.62);
    };
    resize();
    window.addEventListener("resize", resize);
    let raf = 0;
    const t0 = performance.now();
    const THETA0 = Math.PI * 0.74;
    const draw = (now) => {
      const t = (now - t0) / 1000;
      ...
      for (let i = 0; i < stars.length; i++) {
        ...
        let x, y;
        if (s2.orbit) {
          // slow orbital drag around the hole \u2014 space itself is being turned, not just scrolled
          const dx = (s2.px - 0.47) * w, dy = (s2.py - 0.41) * h;
          const rr = Math.hypot(dx, dy);
          const ang = Math.atan2(dy, dx) + t * s2.spin * (1 / (0.35 + rr / R0));
          x = cx + Math.cos(ang) * rr;
          y = cy + Math.sin(ang) * rr;
        } else {
          x = (((s2.px + s2.vx * t) % 1) + 1) % 1 * w;
          y = (((s2.py + s2.vy * t) % 1) + 1) % 1 * h;
        }
        let alpha = s2.a * (0.55 + 0.45 * Math.sin(t * s2.tw * Math.PI + s2.ph));
        if (s2.fade) alpha *= Math.max(0, Math.sin(t * s2.tw * 0.6 + s2.ph));
        if (alpha <= 0.012) continue;
        ctx.globalAlpha = alpha;
        ctx.fillStyle = "#FFFFFF";
        const rr2 = s2.r;
        if (rr2 < 0.8) ctx.fillRect(x, y, 1, 1);
        else {
          ctx.beginPath();
          ctx.arc(x, y, rr2, 0, 6.2832);
          ctx.fill();
        }
      }
      // --- candlestick stream falling into the event horizon ---
      // p is the head's progress; each candle trails it, so the chart stretches, curves and
      // accelerates as the leading candles get closer (r shrinks super-linearly with t).
      const dur = 6.6;
      const raw = Math.min(1.5, t / dur * 1.5);
      const p = reduced ? 0.35 : Math.pow(raw / 1.5, 1.35) * 1.5;
      const k = w * 0.032;
      for (let i = 0; i < CANDLE_N; i++) {
        const ct = p - (CANDLE_N - 1 - i) * 0.021;
        if (ct <= -0.35 || ct >= 1) continue;
        const shrink = Math.pow(Math.max(0.0001, 1 - ct), 1.4);
        const r = R0 * shrink;
        if (r > R0 * 1.25) continue;
        const theta = THETA0 + ct * 1.35;
        const x = cx + Math.cos(theta) * r;
        const y = cy + Math.sin(theta) * r;
        const sc = Math.pow(Math.max(0.0001, 1 - ct), 1.0);
        let alpha = 1;
        if (ct > 0.82) alpha = Math.max(0, (1 - ct) / 0.18);
        if (ct < 0) alpha = Math.max(0, 1 + ct / 0.35);
        if (alpha <= 0.02) continue;
        const c = candles[i];
        const hot = Math.max(0, Math.min(1, (ct - 0.45) / 0.45));
        const base = c.c >= c.o ? [92, 214, 160] : [226, 96, 96];
        const glow = [255, 232, 190];
        const col = `rgb(${Math.round(base[0] + (glow[0] - base[0]) * hot)},${Math.round(base[1] + (glow[1] - base[1]) * hot)},${Math.round(base[2] + (glow[2] - base[2]) * hot)})`;
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(theta + Math.PI / 2);
        ctx.globalAlpha = alpha * 0.92;
        ctx.fillStyle = col;
        const bw = Math.max(0.7, w * 0.019 * sc);
        const wickW = Math.max(0.5, bw * 0.16);
        ctx.fillRect(-wickW / 2, -(c.hi - mid) * k * sc, wickW, (c.hi - c.lo) * k * sc);
        const top = -(Math.max(c.o, c.c) - mid) * k * sc;
        ctx.fillRect(-bw / 2, top, bw, Math.max(0.8, Math.abs(c.c - c.o) * k * sc));
        ctx.restore();
      }
      ctx.globalAlpha = 1;
  ...
  return /* @__PURE__ */ jsxs("div", { className: `splash2-root fixed inset-0 z-50${flare ? " is-flare" : ""}`, style: { opacity: fading ? 0 : 1, pointerEvents: fading ? "none" : "auto", transition: "opacity 900ms cubic-bezier(0.4,0,0.2,1)" }, children: [
    /* @__PURE__ */ jsxs("div", { className: "splash2-bh-scene", children: [
      ...
      /* @__PURE__ */ jsx("div", { className: "splash2-bh-spin", children: /* @__PURE__ */ jsx("div", { className: "splash2-bh-spin-inner" }) }),
      /* @__PURE__ */ jsx("div", { className: "splash2-bh-glow" }),
      /* @__PURE__ */ jsx("canvas", { ref: canvasRef, className: "splash2-space", "aria-hidden": "true" })
    ] }),
...
function MindExe() {
  ...
  useEffect(() => {
    const t1 = setTimeout(() => setSplashFading(true), 7300);
    const t2 = setTimeout(() => setShowSplash(false), 8250);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
    };
      ...
      try {
        ...
        if (profile) {
          ...
          if (restoredEntries.length > 0) {
            setTimeout(() => showToast("\u0414\u0430\u043D\u043D\u044B\u0435 \u0432\u043E\u0441\u0441\u0442\u0430\u043D\u043E\u0432\u043B\u0435\u043D\u044B"), firstLoadRef.current ? 8500 : 300);
          }
        }
  ...
  return /* @__PURE__ */ jsxs("div", { className: `min-h-screen w-full relative theme-fade${accentPreset.cosmic ? " cosmic-theme" : ""}`, style: { background: accentPreset.cosmic ? "#040405" : BASE.bg, fontFamily: "'Inter', sans-serif" }, children: [
    /* @__PURE__ */ jsx("style", { children: `
        ...
        .splash2-space { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 2; }
        .splash2-bh-spin {
